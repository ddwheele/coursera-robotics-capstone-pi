...
def test_tag_in_camera_to_robot_in_world():
  ...
  tag_straight_in_camera = [0,1,mu.to_rad(90)]
  ans = mu.tag_in_camera_to_camera_in_world(tag02_90, tag_straight_in_camera)
  assert tag_angle_is_correct(ans, [0,1,mu.to_rad(90)]) # got 0
  # East
  tag200 =  [2,0,0]
  ans = mu.tag_in_camera_to_camera_in_world(tag200, tag_straight_in_camera)
  assert tag_angle_is_correct(ans, [1,0,0]) # got -90
  # West
  tagMin20_180 = [-2, 0, mu.to_rad(180)]
  ans = mu.tag_in_camera_to_camera_in_world(tagMin20_180, tag_straight_in_camera)
  assert tag_angle_is_correct(ans, [-1,0,mu.to_rad(-180)])

  # South
  tag0Min2_Neg90 = [0, -2, mu.to_rad(-90)]
  ans = mu.tag_in_camera_to_camera_in_world(tag0Min2_Neg90, tag_straight_in_camera)
  assert tag_angle_is_correct(ans, [0,-1,mu.to_rad(-90)])
