...
def tag_angle_is_correct(ans, truth):
  correct = np.allclose(ans[0:1], truth[0:1], rtol=1e-3)
...
def test_tag_in_camera_to_camera_in_world():
  # East
  tag200 =  [2,0,0]
  tag_straight_at_camera_1 = [1,0,mu.to_rad(ZERO_ANGLE_DEG)]
  #import pdb
  #pdb.set_trace()
  ...
  tag_straight_at_camera_root2 = [root2,0,mu.to_rad(ZERO_ANGLE_DEG)]
  ...
  tag_in_camera = [0.5, 0, 0] # straight on
  tag_in_world = [3, 2.5, -1.571]
  import pdb
  # ans is [array([ 3.00010184]), array([ 2.99999999]), -1.571], but fails???
  ans = mu.tag_in_camera_to_camera_in_world(tag_in_world, tag_in_camera)
  print(ans)
  assert triplet_is_correct(ans, [3,3,-1.571])
